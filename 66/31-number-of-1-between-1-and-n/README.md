...
1. 设`step = n % 1000`  
2. 归纳式为：`(n / 1000) * 100 + (if(step >199) 10 else if(step < 100) 0 else step - 100 + 1)`  
后面的依次类推....  

那么我们把个位数上算1的个数的式子也纳入归纳式中：

1. `step = n % 10`  
2. 个位数上1的个数为：`n / 10 * 1 + (if(step > 1) 1 else if(step < 1) 0 else step - 1 + 1)`   

完美！归纳式看起来已经很规整了。来一个更抽象的归纳，设 bit 为计算 1 所在的位数，`bit = 1`表示计算个位数的1的个数，`bit = 10`表示计算十位数的1的个数等等：  

1. `step = n % (bit * 10)`  
2. `count(bit) = (n / (bit * 10)) * bit + (if(step > bit * 2 - 1) bit else if(step < bit) 0 else step - bit + 1)`  
